...
const Course = ({
  ...
  <motion.div
    ...
    className='bg-[#1d1836] p-10 rounded-3xl xs:w-[400px] xs:h-[500px] w-full'
  >
      ...
      <div className = "flex md:flex-row flex-col justify-around">
        ...
        <div className= "mb-20 flex flex-col gap-7">
          {coursework.map((course, index) => (
              <div className="bg-[#1d1836] cursor-pointer rounded-lg p-4 text-2xl color: rgb(0 0 0)" key = {index} onMouseEnter={() => setCourses(index)}>{course.title}</div>
          ))}
        </div>
      </div>
